...
CLIENT_SECRETS_FILE = "credentials_client.json"
...
def TestView(request):
    ...
    return Response({"test": "django is working"})
...
def GoogleCalendarRedirectView(request):
    ...
    credentials_dict = lambda credentials: {
    ...
    request.session['credentials'] = credentials_dict(credentials)
    ...
    events_list_append = []
    if not calendar_events['items']:
        print('No event in list')
        return Response({"message": "No item found."})
    else:
        for events_list in calendar_events['items']:
            events_list_append.append(events_list)
            return Response({"events": events_list_append})
    return Response({"error": "calendar events not here"})
